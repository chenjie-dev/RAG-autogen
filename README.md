...
- **功能简介**：
  - 系统会自动分析用户输入的查询意图，并生成多个优化版本
  - 支持多种重写策略：扩展版本（添加相关术语）、结构化版本（拆分为具体查询）、简化版本（保留核心信息）
  - 提供查询意图分析和相关查询建议

- **工作流程**：
  1. 用户输入原始查询
  2. 系统分析查询意图（定义、方法、原因、比较、数据等）
  3. 生成多个重写版本，每个版本使用不同策略
  4. 使用最优的重写版本进行检索
  5. 在Web界面显示查询建议和重写结果
...
- **API接口**：
  - `POST /api/query_suggestions`: 获取查询建议和重写结果
  - 支持传统RAG和AutoGen两种系统模式

### 测试说明

- 主要测试脚本已删除，建议通过Web界面或API端到端验证。
- 文档处理相关测试仍保留在`tests/`目录下。
- 如需自定义测试，可参考`src/utils/reranker.py`、`src/utils/vector_store.py`和`src/utils/query_rewriter.py`中的方法。
...
**更新内容**: 
- ✅ 新增查询重写功能：智能优化用户问题输入，提升检索效果
- ✅ 新增查询意图分析：自动识别查询类型，提供相关建议
- ✅ 新增查询建议功能：支持多种重写策略，提供查询优化选项
